...
if not selected:
    selected = ["ETH", "TRUMP", "FARTCOIN", "CHILLGUY", "BERA"]
...
# ——— Build an evolving area graph with px.area ———
fig = px.area(
    vol_long,
    x="Date",
    y="Volume",
    color="Token",
    title="Token Listing Dates and Evolving Daily Volumes",
    animation_frame="Date",  # Add animation frame for evolving graph
    range_y=[0, vol_long["Volume"].max() * 1.1]  # Set consistent y-axis range
)
...
#     mode="lines",
#     line=dict(color="orange", dash="dash"),
#     hovertemplate=f"<b>{row['Token']}</b><br>Date: {d}<extra></extra>",
#     showlegend=False
# ))
